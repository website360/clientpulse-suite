...
Deno.serve(async (req) => {
  ...
  try {
    console.log('Starting recurring payables generation...');
    ...
    // Find recurring payables where the last charge expires within a month
    const { data: existingPayables, error: fetchError } = await supabase
    ...
    if (fetchError) {
      console.error('Error fetching payables:', fetchError);
      throw fetchError;
    }

    console.log(`Found ${existingPayables?.length || 0} recurring payables`);

    // Group by supplier_id, description, category, occurrence_type
    const grouped = new Map<string, Payable[]>();
    
    existingPayables?.forEach((payable) => {
      const key = `${payable.supplier_id}-${payable.description}-${payable.category}-${payable.occurrence_type}`;
      if (!grouped.has(key)) {
        grouped.set(key, []);
      }
      grouped.get(key)!.push(payable as Payable);
    });
    ...
    for (const [key, payables] of grouped.entries()) {
      // Get the latest charge
      const latestCharge = payables[0]; // Already sorted by due_date desc
      ...
      if (latestDueDate <= oneMonthFromNow) {
        ...
        for (let i = 1; i <= 12; i++) {
          ...
          newCharges.push({
            supplier_id: latestCharge.supplier_id,
    ...
    console.log(`Successfully generated ${totalGenerated} new recurring payables`);

    return new Response(
      JSON.stringify({
        success: true,
        generated: totalGenerated,
        message: `Generated ${totalGenerated} recurring payables`,
      }),
      {
    ...
    console.error('Error in generate-recurring-payables:', error);
    return new Response(
      JSON.stringify({
        success: false,
