...
DECLARE
  ...
  i INTEGER;
  base_num INTEGER := 100000; -- Base para garantir CPF/CNPJ únicos
BEGIN
  ...
  -- Criar 150 clientes com CPF/CNPJ únicos
  FOR i IN 1..150 LOOP
    INSERT INTO clients (
      client_type,
      full_name,
    ...
    VALUES (
      ...
      'Cliente ' || i,
      'cliente' || (base_num + i) || '@example.com',
      '11' || LPAD((base_num + i)::TEXT, 9, '9'),
      CASE WHEN i % 2 = 0 THEN LPAD((base_num + i)::TEXT, 14, '0') ELSE LPAD((base_num + i)::TEXT, 11, '0') END,
      LPAD((base_num + i)::TEXT, 8, '0'),
      'Rua Exemplo ' || i,
      i::TEXT,
      'Centro',
      'São Paulo',
  ...
  -- Buscar IDs dos 150 clientes mais recentes
  SELECT ARRAY_AGG(id) INTO client_ids FROM (
    SELECT id FROM clients ORDER BY created_at DESC LIMIT 150
  ) AS recent_clients;
