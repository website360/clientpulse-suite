...
-- RLS para audit logs (somente admins podem visualizar)
...
-- Tabela de permissões (ações possíveis em cada módulo)
...
-- Permissões padrão (CRUD para cada módulo)
...
-- Permissões especiais para módulo financeiro
INSERT INTO public.permissions (module_id, action, description, requires_approval)
SELECT 
  m.id,
  actions.action,
...
-- Tabela de atribuição de roles customizadas aos usuários
...
-- Tabela de sessões ativas para controle
