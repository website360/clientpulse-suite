-- Ajustar fluxo: não marcar whatsapp_sent na criação; usar trigger pelos logs
-- 1) Atualiza a função existente com assinatura (p_plan_id uuid, p_next_date date, p_notes text, p_items jsonb)
...
BEGIN
  ...
  -- Inserir execução de manutenção (não marcar whatsapp_sent aqui)
  ...
  -- Importante: NÃO atualizar whatsapp_sent aqui. 
  -- Ele será atualizado automaticamente pelo trigger quando houver log 'whatsapp' = 'sent'.

  RETURN v_exec_id;
END;
$$;

-- 2) Criar função de trigger para atualizar whatsapp_sent quando houver log 'sent' do WhatsApp
CREATE OR REPLACE FUNCTION public.update_maintenance_execution_whatsapp_status()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
...
BEGIN
  IF NEW.reference_type = 'maintenance'
     AND COALESCE(NEW.channel, '') = 'whatsapp'
...
-- 3) Criar trigger em notification_logs (insere/atualiza)
DROP TRIGGER IF EXISTS trg_update_exec_whatsapp_status ON public.notification_logs;
CREATE TRIGGER trg_update_exec_whatsapp_status
AFTER INSERT OR UPDATE ON public.notification_logs
FOR EACH ROW
EXECUTE FUNCTION public.update_maintenance_execution_whatsapp_status();