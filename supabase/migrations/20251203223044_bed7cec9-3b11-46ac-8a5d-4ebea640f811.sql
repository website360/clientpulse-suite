-- Atualizar a função RPC para aceitar o parâmetro p_send_whatsapp
CREATE OR REPLACE FUNCTION public.create_maintenance_execution(
  p_plan_id uuid, 
  p_next_date date, 
  p_notes text, 
  p_items jsonb,
  p_send_whatsapp boolean DEFAULT TRUE
)
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- SÓ enviar notificação se p_send_whatsapp = TRUE
  IF p_send_whatsapp THEN
  ...
  ELSE
    -- Se não enviar, garantir que fica marcado como não enviado
    UPDATE maintenance_executions 
    SET whatsapp_sent = FALSE, whatsapp_sent_at = NULL 
    WHERE id = v_exec_id;
  END IF;
