-- Drop trigger e função antigas com CASCADE (timing incorreto)
DROP TRIGGER IF EXISTS on_maintenance_completed ON maintenance_executions CASCADE;
DROP FUNCTION IF EXISTS public.trigger_maintenance_completed() CASCADE;

-- Recriar função RPC com notificação integrada no final
...
BEGIN
  ...
  -- AGORA buscar dados para notificação (items já foram salvos!)
  SELECT 
    jsonb_build_object(
  ...
  -- Montar checklist formatado (agora os items existem no banco!)
  SELECT STRING_AGG(
    CASE 
  ...
  -- Enviar notificação com checklist completo
