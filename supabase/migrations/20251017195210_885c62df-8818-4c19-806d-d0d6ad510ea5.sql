-- Corrigir função para usar os valores corretos do enum
...
BEGIN
  ...
  -- Determinar novo status (usando valores corretos do enum)
  IF v_total_items = 0 THEN
    v_new_status := 'pendente';
  ELSIF v_completed_items = 0 THEN
    v_new_status := 'pendente';
  ELSIF v_completed_items = v_total_items THEN
    v_new_status := 'concluida'; -- Corrigido para feminino
    ...
    SET 
      ...
      updated_at = NOW()
    WHERE id = v_stage_id AND status != 'concluida';
